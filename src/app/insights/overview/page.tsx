...
const METRIC_ANALYSIS: Record<string, { prompt: string; response: string }> = {
  ...
  'volatile-0': {
    prompt: 'Analyse AI meeting facilitation (+9% confidence)',
    response: `**AI meeting facilitation is the fastest-moving theme this period — confidence up +9%, now at 78%.**\n\nSignal is building across HR, L&D, and enterprise workshop facilitators. The pattern is multi-source: Gong call mentions, community posts, and survey responses are all moving in the same direction within the same two-week window. That kind of convergence is a strong indicator the momentum is real.\n\n**What's driving it**: teams running large-scale retrospectives and workshops are reporting that AI-generated agendas and timers would eliminate 60% of pre-session prep work. The use case is specific enough to be buildable and broad enough to justify investment.\n\n**ARR signal**: $3.8M ARR at stake across the accounts currently flagging this. Two enterprise accounts added it as a blocker in the last sprint.\n\n**Recommended action**: this theme is ready for a prioritisation conversation. Confidence is on a clear upward trajectory — waiting another cycle risks the window narrowing as competitors catch up.`,
  },
  'volatile-1': {
    prompt: 'Analyse template library depth (+8% confidence)',
    response: `**Template library depth is gaining momentum — confidence up +8% this period, now at 76%.**\n\nSales call analysis is the primary signal driver. Prospects in mid-market evaluations are comparing template counts directly against Lucidspark and Conceptboard — and the gap is measurable. Current state is 340 templates vs 600+ at key competitors.\n\n**Why this matters now**: the signal is coming primarily from deals in progress, not post-sale feedback. That means it's actively influencing purchase decisions. A gap this visible in competitive evaluations tends to compound over time.\n\n**What's changed**: confidence climbed +8% because three new sales call transcripts this period explicitly named template depth as a deciding factor. It's moved from background signal to a documented deal risk.\n\n**Recommended action**: review this alongside the sales team. Short-term, a template expansion sprint could be a high-ROI investment to close competitive gaps before Q2 renewal cycles begin.`,
  },
  'volatile-2': {
    prompt: 'Analyse real-time translation (+8% confidence)',
    response: `**Real-time translation for global teams is an emerging breakout — confidence up +8%, now at 71%.**\n\nThe signal is concentrated in APAC and LATAM expansion cohorts — multinational enterprises where language is a documented collaboration barrier. Sticky note and comment auto-translation is the specific ask, not full interface localisation.\n\n**Why this is interesting**: this is early-stage signal that's moving fast. Themes that start below 75% confidence and climb 8% in a single period often continue accelerating as the initial signal attracts more customer conversations.\n\n**Competitive context**: no major whiteboard competitor has shipped this feature. There's a clear window to differentiate — especially for enterprise deals where global team composition is a common org structure.\n\n**Recommended action**: flag for the next roadmap input cycle. Signal quality is high and the competitive window is open. A lightweight discovery sprint with APAC account managers could validate the scope before committing to build.`,
  },
  'Customer Sentiment': {
    ...
    <div className="relative h-full w-full flex flex-col" style={{ backgroundColor: '#FBFAF7' }}>
      ...
      >
        <main className="px-0 py-[60px] mx-[60px]">
          ...
          {/* Second charts row */}
          <div className={`grid mb-9 ${aiOpen ? 'grid-cols-1 gap-9' : 'grid-cols-2 gap-9'}`}>
            <VolatileSignals onOpenChat={() => openMetric('Volatile Signals')} onViewAnalysis={(i) => openMetric(`volatile-${i}`)} />
